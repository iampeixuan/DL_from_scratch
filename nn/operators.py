...
class conv(operator):
    ...
    def forward(self, input, weights, bias):
        ...
        kernel_rows = weights.reshape((out_channel, in_channel * kernel_h * kernel_w))
        ...
        output = np.array([(np.dot(kernel_rows, i).T + bias).T.reshape((out_channel, out_height, out_width)) for i in input_conv])
...
class dropout(operator):
    ...
    def forward(self, input):
        ...
        if self.training:
            ...
            # p is the probability to decide whether to drop or not
            self.mask = (p >= self.rate).astype('int')
            output = np.multiply(self.mask, input) * scale
        else:
            output = input
        return output
    ...
    def backward(self, out_grad, input):
        ...
        if self.training:
            scale = 1/(1-self.rate)
            in_grad = np.multiply(self.mask, out_grad) * scale
        else:
            in_grad = out_grad
        return in_grad
